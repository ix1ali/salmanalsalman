 ...
 *
 * يُطبع في صفحة A4 واحدة كالنموذج القديم: الورقة بمقاس الصفحة تمامًا،
 * ويُصغَّر الخط تلقائيًا حتى يتّسع النص كله فلا ينتقل شيء لصفحة ثانية.
 */
const FS_MAX = 12.5;
const FS_MIN = 7.5;
const NBSP = " ";

/** قيمة مُعبّأة داخل النص، أو فراغ يُكتب باليد. */
const Fill = ({ children }: { children?: React.ReactNode }) => (
  <b style={{ color: INK, padding: "0 .15em" }}>{children || NBSP.repeat(8)}</b>
);
...
export function ContractSheet({ f }: { f: ContractFields }) {
  ...
  const [fs, setFs] = useState(FS_MAX);

  // نبدأ من أكبر خط ونصغّره حتى لا يتجاوز المحتوى ارتفاع الصفحة.
  useLayoutEffect(() => {
    ...
    const fit = () => {
      ...
      while (el.scrollHeight > el.clientHeight + 1 && size > FS_MIN) {
        size -= 0.25;
        el.style.fontSize = `${size}px`;
      }
      setFs(size);
  ...
  const cell: React.CSSProperties = { border: `1px solid ${INK}`, padding: ".3em .6em", verticalAlign: "top" };
  const dots = <p style={{ marginTop: "1.6em", color: "#6f7f92", letterSpacing: ".25em" }}>. . . . . . . . . . . . . . . . . .</p>;

  const clauses: React.ReactNode[] = [
    <>
      مدة هذا العقد ( <Fill>{f.duration}</Fill> ) يبدأ بتاريخ <Fill>{dateOrBlank(f.startDate)}</Fill> وينتهي بتاريخ{" "}
      <Fill>{dateOrBlank(f.endDate)}</Fill> وتتجدد تلقائيًا لمدد مماثلة ما لم يخطر أي من الطرفين الطرف الآخر بعدم رغبته
      في التجديد كتابةً قبل انتهاء المدة بشهر على الأقل، وعند حصول التنبيه بالإخلاء فإنه يجب على المستأجر أن يسهّل
      ويسمح بدخول كل من يرغب بمعاينة العين.
    </>,
    <>
      القيمة الإيجارية الشهرية <Fill>{f.rent ? KWD(f.rent) : ""}</Fill> تعتبر دينًا مترصدًا في ذمة الطرف الثاني محدد
      القيمة وواجب الوفاء.
      {f.deposit ? <> والتأمين المدفوع <Fill>{KWD(f.deposit)}</Fill>.</> : null}
    </>,
    ...
    <>
      ...
      ( <Fill>{f.occupants ? num(f.occupants) : ""}</Fill> ) شخص / أشخاص.
    </>,
    <>
  ...
  return (
    <div className="a4-scroll">
      <div
        ...
        className="a4-page"
        style={{
          width: "210mm", height: "297mm", padding: "10mm 12mm", boxSizing: "border-box",
          overflow: "hidden", display: "flex", flexDirection: "column",
          fontSize: fs, lineHeight: 1.55, color: "#1d2f47", background: "#fff",
        }}
      >
        {/* ------------------------------ العنوان ------------------------------ */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "start", marginBottom: ".6em" }}>
          <div>
            <p style={{ fontWeight: 800, color: INK, fontSize: "1.05em" }}>{data.settings.orgName}</p>
            <p style={{ color: MUTED, fontSize: ".85em" }}>عقاري / {owner}</p>
          </div>
          <div style={{ textAlign: "center" }}>
            <p style={{ fontWeight: 800, color: INK, fontSize: "1.55em", textDecoration: "underline", lineHeight: 1.2 }}>عقد إيجار</p>
            <p style={{ fontWeight: 800, color: INK, fontSize: "1.2em", textDecoration: "underline", direction: "ltr" }}>Rent Contract</p>
          </div>
          <div style={{ textAlign: "left", direction: "ltr" }}>
            <p style={{ fontWeight: 800, color: INK }}>Salman AlSalman</p>
            <p style={{ color: MUTED, fontSize: ".85em" }}>Real Estate</p>
          </div>
        </div>
        ...
        {/* ------------------------------ الطرفان ------------------------------ */}
        <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", marginBottom: ".5em" }}>
          <tbody>
            <tr>
              <td style={{ ...cell, width: "50%" }} rowSpan={5}>
                <p style={{ fontWeight: 800, color: NAVY }}>الطرف الأول ( المؤجر ) :</p>
                <p style={{ fontWeight: 700, color: INK }}>السيد / {owner}</p>
                <p style={{ color: MUTED }}>بصفته مالك العقار</p>
              </td>
              <td style={cell}>
                <span style={{ fontWeight: 800, color: NAVY }}>الطرف الثاني ( المستأجر ) : </span>
                <Fill>{f.tenantName}</Fill>
              </td>
            </tr>
            <tr><td style={cell}>الرقم المدني : <Fill>{f.civilId}</Fill></td></tr>
            <tr><td style={cell}>الجنسية : <Fill>{f.nationality}</Fill></td></tr>
            <tr><td style={cell}>المهنة : <Fill>{f.job}</Fill></td></tr>
            <tr><td style={cell}>رقم الهاتف : <Fill>{f.phone}</Fill></td></tr>
          </tbody>
        </table>

        <p style={{ fontWeight: 700, marginBottom: ".35em" }}>
          على أن يؤجر الطرف الأول للطرف الثاني ( العين ) الواقعة بمنطقة ( <Fill>{f.area}</Fill> ) قطعة رقم
          ( <Fill>{f.block}</Fill> ) شارع ( <Fill>{f.street}</Fill> ) عمارة رقم ( <Fill>{f.buildingNo}</Fill> ) الدور
          ( <Fill>{f.floor}</Fill> ) شقة رقم ( <Fill>{f.unitNo}</Fill> ) كسكن خاص له ولعائلته وفق الشروط التالية:
        </p>

        {/* ------------------------------- البنود ------------------------------- */}
        <ol style={{ textAlign: "justify" }}>
          {clauses.map((c, i) => (
            <li key={i} style={{ display: "flex", gap: ".45em", marginBottom: ".12em" }}>
              <span style={{ fontWeight: 800, color: INK, minWidth: "1.4em" }}>{num(i + 1)}.</span>
        ...
        {/* ------------------------------- التوقيع ------------------------------- */}
        <div style={{ marginTop: "auto", paddingTop: ".8em", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2em", textAlign: "center", fontWeight: 700, color: INK }}>
          <div>
            <p>الطرف الأول</p>
            <p>( المؤجر )</p>
            <p style={{ marginTop: ".2em" }}>{owner}</p>
            {dots}
          </div>
          <div>
            <p>الطرف الثاني</p>
            <p>( المستأجر )</p>
            <p style={{ marginTop: ".2em" }}>{f.tenantName || NBSP}</p>
            {dots}
          </div>
        </div>
      </div>
